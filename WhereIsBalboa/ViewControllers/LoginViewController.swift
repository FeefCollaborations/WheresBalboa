...
class LoginViewController: UIViewController, UITextFieldDelegate, AccountEditor {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        // TODO: Remove hiding cohort text field for multiple cohort support
                ...
                UserManager.new(for: loginInfo.cohort, withLoggedInUser: loggedInUser) { result in
                    DispatchQueue.main.async {
                        strongSelf.dismiss(animated: true) {
                            switch result {
                                case .failure:
                                    strongSelf.showOneOptionAlert(title: "Shit!", message: "Something went really wrong. Please try logging in again later.")
